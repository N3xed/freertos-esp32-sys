...
impl Builder {
    ...
    pub fn new() -> Builder {
        let b = Builder {
            freertos_dir: PathBuf::default(),
            freertos_config_dir: PathBuf::default(),
            freertos_shim: PathBuf::default(),
            freertos_port: None,
            cc: cc::Build::new(),
            heap_c: None,
        };
    ...
    fn freertos_port_files(&self) -> Vec<String> {
        let files: Vec<_> = WalkDir::new(self.get_freertos_port_dir())
            ...
            .filter_map(|entry| {
                ...
                if f_name.ends_with(".c") || f_name.to_lowercase().ends_with(".s") {
                    return Some(String::from(entry.path().to_str().unwrap()));
                }
                return None;
            })
    ...
    fn get_freertos_port_dir(&self) -> PathBuf {
        ...
        if self.freertos_port.is_some() {
            return PathBuf::from(self.freertos_port.as_ref().unwrap());
        }
    ...
    pub fn compile(&self) -> Result<(), Error> {
        ...
        b.include(cargo_emit(&self.get_freertos_port_dir()));
        b.include(cargo_emit(&self.freertos_config_dir.clone()));
        if let Some(heap_c) = self.heap_c_file() {
            b.file(heap_c);
        }
        ...
        let res = b.try_compile("freertos-sys");
        if res.is_err() {
            return Err(anyhow!("{}", res.unwrap_err()));
        }
